const ReservedItem = require("../models/ReservedModel");
...
const createItem = async (req, res, next) => {
  try {
    ...
    const newItem = new ReservedItem(req.body);
    await newItem.save();
...
const getItemByImage = async (req, res, next) => {
  try {
    ...
    const item = await ReservedItem.findOne({ image });
    if (!item) {
...
const getItemById = async (req, res, next) => {
  try {
    ...
    const item = await ReservedItem.findById(id);
    if (!item) {
      return res.status(404).json({ message: "Item not found" });
    }
